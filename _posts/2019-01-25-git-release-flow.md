...
If no release has been planned, the work should continue on the master branch. Remember that the master branch cannot be directly modified. This is the philosophy of the **feature branch approach** that is the minimum requirement that one should fulfill to correctly employ a proper git workflow: when the task *foo* should be developed, the developer should create the *foo* branch from the master branch and complete the task on that very branch (note: the development should involve only changes for that very task, please refrain from any desire to fix other things you notice during the progress of that task but instead open a new Jira task (or whatever) to fix that issue after you will be finished with this current one). Once the task is completed, the developer should pull from the master branch into his *foo* branch any new changes in order to solve locally any conflicts and then finally he should do a pull/merge request that will be reviewed and then applied to the master branch. In Figure 1 this it is described through the use of the light blue lines. 

If, most commonly, the release of a 2.0.0 version has been planned (e.g.: M129 as in Figure 1), a new branch should be created starting from the master. Everyone that is intended to work on the 2.0.0 release should work directly, but always with the feature branch approach, on this branch. Please note that creating a 2.0.0 release branch does not mean that that version has been released to the public. Here lie some important aspects of this Release Flow workflow:
...
The *develop* branch should replicate the master and exists to keep always the master branch untouched. What I said in the previous chapter (Release Flow - Version 1) it is still true here but instead, to be applied to the master branch it is referred to the develop branch. What happens here are the following: 
...
- No new features should be added to the current release branch as for version 1.
...
# Conclusion
I've had the chance to work with this workflow in my previous working experience where we developed a lot of industrial ceramic printer machine models and for every machine, we had a lot of software versions for which we had to ensure stability and efficiency because of the important performance and reliability requirements of ceramic production lines. Moreover, every client had a different version of the software (someone was accustomed to one specific version, someone has an old machine that could not be updated anymore etc...) so we had to maintain every single version. Thanks to the approach presented we were able to maintain a lot of different software versions and work with big and heterogeneous teams having complete control of the code. The main pros are:
